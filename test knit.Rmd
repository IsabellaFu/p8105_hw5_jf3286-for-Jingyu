...
Power increases as effect size increases. The slope (increasing speed) is highest among effect size 2,3,4,5, and the incrasing speed (slope) slows down a bit when effect size increases from 5 to 6. 
...
```
They are not the same. Because average estimate of samples whose null are rejected are mostly larger than true value of beta1(except for true value equals 0). This difference  might be cause by the missing of  true value of beta1 whose p_values are larger than 0.05. Those missing  beta 1 may have samller estimates than true value and missing them may coz a increase in estimate of beta1 corresponding to each true value. 
